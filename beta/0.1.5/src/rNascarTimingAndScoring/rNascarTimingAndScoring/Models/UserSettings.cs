...
namespace rNascarTimingAndScoring.Models
{
    public class UserSettings
    {
        ...
        {
            var settingsDirectory = $"{Path.GetDirectoryName(Application.ExecutablePath)}\\settings\\";

            if (!Directory.Exists(settingsDirectory))
                Directory.CreateDirectory(settingsDirectory);

            return $"{Path.GetDirectoryName(Application.ExecutablePath)}\\settings\\rNascarUserSettings.json";
        }
    }
}
